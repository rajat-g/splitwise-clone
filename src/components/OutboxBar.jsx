import { opBelongsTo } from "../lib/offline";
...
function ForeignNotice({ count }) {
  if (!count) return null;
  return (
    <p className="mt-2.5 flex items-start gap-2 rounded-xl bg-white/70 px-3 py-2 text-[13px] text-slate-600 dark:bg-white/[0.04] dark:text-slate-300">
      <Icon.Shield className="mt-0.5 h-4 w-4 shrink-0 text-slate-400 dark:text-slate-500" />
      <span>
        <b className="tnum">{count} change{count === 1 ? "" : "s"}</b> queued under a different
        signed-in account on this device — {count === 1 ? "it stays" : "they stay"} untouched until
...
export function OutboxBar({ items, userId, online, syncing, onSync, onRetry, onDiscard }) {
  ...
  // Identity boundary: queued operations belong to one signed-in account.
